...
app.controller('PersonasController', function($scope, $http, UsuarioFactory, UtilsService, $rootScope, PersonaService, DepartamentoService, $filter, $route, CONST, $location, ParametroService) {
    ...
    $scope.editarPersona = function(item) {
        if (!angular.isUndefined(item)) {
            var foundItem = $filter('filter')($scope.gridOptions.data, {phb_id: item.phb_id}, true)[0];
    ...
    $scope.querySearch = function(query) {
        ...
        return result;*/
		if (!query) return $scope.states;

		var lowercaseQuery = query.toLowerCase();
		var splitQuery = lowercaseQuery.split(/\s+/).sort();

		return $scope.states.filter(function(state) {
			var lowercaseValue = state.value.toLowerCase();
			var splitValue = lowercaseValue.split(/[\s,]+/).sort();

			return splitQuery.every(function(queryWord) {
				return splitValue.some(function(valueWord) {
					return valueWord.startsWith(queryWord);
				});
			});
		});
    };
    ...
    $scope.aceptar = function(silent) {
		...
		if (!UtilsService.isEmpty($scope.phb_fon_first_name) && $scope.phb_fon_first_name != "") {
			...
			if (UtilsService.hasUppercase($scope.phb_fon_first_name)) {
				UtilsService.showToast({delay: 5000, text: "La fonética del primer nombre no puede contener mayúsculas (solapa reconoc. de voz)"});
				return;
			}
		}
		if (!UtilsService.isEmpty($scope.phb_fon_last_name1) && $scope.phb_fon_last_name1 != "") {
			...
			if (UtilsService.hasUppercase($scope.phb_fon_last_name1)) {
				UtilsService.showToast({delay: 5000, text: "La fonética del primer apellido no puede contener mayúsculas (solapa reconoc. de voz)"});
				return;
			}
		}
		if (!UtilsService.isEmpty($scope.phb_fon_middle_name) && $scope.phb_fon_middle_name != "") {
			...
			if (UtilsService.hasUppercase($scope.phb_fon_middle_name)) {
				UtilsService.showToast({delay: 5000, text: "La fonética del segundo nombre no puede contener mayúsculas (solapa reconoc. de voz)"});
				return;
			}
		}
		if (!UtilsService.isEmpty($scope.phb_fon_last_name2) && $scope.phb_fon_last_name2 != "") {
			...
			if (UtilsService.hasUppercase($scope.phb_fon_last_name2)) {
				UtilsService.showToast({delay: 5000, text: "La fonética del segundo apellido no puede contener mayúsculas (solapa reconoc. de voz)"});
				return;
			}
		}
        ...
        var data = {
            ...
            phb_first_name: $scope.phb_first_name.trim(),
            phb_last_name1: $scope.phb_last_name1.trim(),
            phb_nick_name: $scope.phb_nick_name,
            phb_dep_id: $scope.phb_dep_id,
            phb_middle_name: ($scope.phb_middle_name == null)? '' : $scope.phb_middle_name.trim(),
            phb_last_name2: ($scope.phb_last_name2 == null)? '' : $scope.phb_last_name2.trim(),
